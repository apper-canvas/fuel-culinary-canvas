...
function Home({ recipeFormRef }) {
  ...
  const handleAddRecipe = async (recipe) => {
    ...
    /* Replaced with backend operation
  ...
  const handleDeleteRecipe = (id) => {
    ...
    // Close the detail view if we're deleting the recipe we're viewing
      ...
      setViewingRecipe(null);
        <h1 className="text-2xl md:text-3xl font-bold mb-4 text-surface-800 dark:text-surface-100">Welcome, {user?.firstName || 'Chef'}</h1>
    ...
    <div className="container mx-auto px-4 py-6 md:py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      <span key={cat} className="text-xs font-medium bg-primary/80 text-white px-2 py-1 rounded-full">
                        {cat.split(',').map(c => c.charAt(0).toUpperCase() + c.slice(1)).join(', ')}
            <motion.div
              key={recipe.id}
              ...
              <p className="text-surface-600 dark:text-surface-400 text-sm mb-4 line-clamp-2">
                {recipe.description}
                ? "Start adding your favorite recipes using the New Recipe button." 
